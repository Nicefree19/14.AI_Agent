...
async def fetch_new_messages():
    ...
    try:
        ...
        updates = await bot.get_updates(
            offset=last_update_id + 1, timeout=5, allowed_updates=["message"]
        )
        ...
        for update in updates:
            ...
            if msg.audio:
                file_path = await download_file(
                    bot, msg.audio.file_id, msg.message_id, "audio", msg.audio.file_name
                )
                if file_path:
                    files.append(
                        {
            ...
            if msg.location:
                ...
                # 선택적 정보 (있으면 추가)
                if (
                    hasattr(msg.location, "horizontal_accuracy")
                    and msg.location.horizontal_accuracy
                ):
                    location_info["accuracy"] = msg.location.horizontal_accuracy

                print(
                    f"📍 위치 수신: 위도 {msg.location.latitude}, 경도 {msg.location.longitude}"
                )
            ...
            for msg in new_messages:
                text_preview = (
                    ...
                    if msg["text"]
                        ...
                        "(파일만)"
                        if msg["files"]
                        else "(위치)" if msg.get("location") else ""
                    )
                )
                file_info = f" + {len(msg['files'])}개 파일" if msg["files"] else ""
                location_info = f" + 위치 정보" if msg.get("location") else ""
                print(
                    f"📨 새 메시지: [{msg['timestamp']}] {msg['first_name']}: {text_preview}...{file_info}{location_info}"
                )
...
# ── PID 파일 기반 Listener 활성 감지 ──────────────────────────

def _write_pid_file():
    """현재 프로세스 PID를 파일에 기록."""
    try:
        os.makedirs(os.path.dirname(LISTENER_PID_FILE), exist_ok=True)
...
def is_listener_running() -> bool:
    """PID 파일을 확인하여 Listener 데몬이 실행 중인지 판별.

    PID 파일이 존재하고, 해당 프로세스가 살아있으면 True.
    프로세스가 죽었으면 stale PID 파일을 정리하고 False.
    """
    ...
    try:
        ...
        # Windows: OpenProcess로 프로세스 생존 확인
        import ctypes
    ...
    try:
        while True:
            ...
            try:
                ...
                _write_pid_file()  # 매 사이클마다 PID 파일 갱신 (liveness proof)

                if result is None:
                    consecutive_errors += 1
                    wait = min(backoff_base * (2 ** consecutive_errors), max_backoff)
                    print(f"[{now}] #{cycle_count} - 오류 발생, {wait}초 후 재시도... (연속 {consecutive_errors}회)")
                    if consecutive_errors >= 5:
                        print(f"[{now}] ⚠️ 연속 {consecutive_errors}회 실패 — 네트워크 상태를 확인하세요")
                ...
                consecutive_errors += 1
                wait = min(backoff_base * (2 ** consecutive_errors), max_backoff)
                print(f"[{now}] #{cycle_count} - ❌ 예외: {e} ({wait}초 후 재시도)")
                await asyncio.sleep(wait)
                continue
